        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="flex items-center justify-between h-16 md:h-20">
            ...
            <div className="flex items-center gap-2 md:gap-3">
              ...
              {visitorName ? (
                <div className="hidden md:flex items-center gap-2">
                  <Link href={dashboardHref}
                    ...
                    {isStaff && <span className="text-xs bg-ice-600 text-white px-1.5 py-0.5 rounded-full">{visitorRole === 'admin' ? t('nav_admin') : t('nav_instructor_role')}</span>}
                    {!isStaff && immersionSummary?.next_date && (
                      <span className="flex items-center gap-1 text-xs bg-ice-900/60 text-ice-300 border border-ice-700/40 px-2 py-0.5 rounded-full">
                        ...
                        {immersionSummary.sessions_remaining !== null && (
                          <span className="text-ice-400 font-bold">· {t('nav_sessions_left').replace('{n}', String(immersionSummary.sessions_remaining))}</span>
                        )}
                      </span>
                    )}
                  </Link>
              ...
              {/* WhatsApp icon */}
              <WhatsAppLink href="https://wa.me/972552482441" source="Header"
                ...
                {a11yOpen && (
                  ...
                  >
                    <div className="flex items-center justify-between mb-4">
                      <h2 className="font-black text-navy-900 text-base">{t('a11y_title')}</h2>
                      <button onClick={() => setA11yOpen(false)} aria-label={t('a11y_close')}
                        className="text-slate-400 hover:text-slate-700 text-xl leading-none">✕</button>
                    </div>
                    <div className="mb-4">
                      <p className="text-xs font-semibold text-slate-500 mb-2">{t('a11y_font_size')}</p>
                      <div className="flex gap-2">
                        {[{ label: t('a11y_normal'), val: 0 }, { label: t('a11y_large'), val: 1 }, { label: t('a11y_xlarge'), val: 2 }].map(opt => (
                    ...
                    <div className="space-y-2 mb-4">
                      <button onClick={() => setContrast(c => !c)}
                        ...
                        <span>{t('a11y_contrast')}</span><span>{contrast ? '✓' : ''}</span>
                      </button>
                      <button onClick={() => setUnderline(u => !u)}
                        ...
                        <span>{t('a11y_underline')}</span><span>{underline ? '✓' : ''}</span>
                      </button>
                    </div>
                    ...
                    <div className="border-t border-slate-100 pt-3 space-y-1">
                      <Link href="/accessibility" onClick={() => setA11yOpen(false)}
                        className="block text-sm text-blue-700 hover:underline">{t('a11y_statement')}</Link>
              ...
              <button className="lg:hidden text-white p-2 rounded-lg hover:bg-navy-700 transition-colors"
                ...
                aria-label={menuOpen ? t('a11y_close') : t('a11y_open')} aria-expanded={menuOpen}>
